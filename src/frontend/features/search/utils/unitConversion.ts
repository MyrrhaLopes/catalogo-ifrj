...
export function getDisplayUnits(baseUnit: "meter" | "minute"): DisplayUnit[] {
  if (baseUnit === "meter") return ["cm", "m", "km"];
  return ["s", "min", "h"];
}

export function toBaseUnit(value: number, displayUnit: DisplayUnit, baseUnit: "meter" | "minute"): number {
  if (baseUnit === "meter") {
...
export function fromBaseUnit(value: number, displayUnit: DisplayUnit, baseUnit: "meter" | "minute"): number {
  if (baseUnit === "meter") {
    const factor = METER_TO_BASE[displayUnit as MeterDisplayUnit];
    if (factor === undefined) throw new Error(`Unidade inválida para meter: ${displayUnit}`);
    return value / factor;
